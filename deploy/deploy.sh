...
if [ $name == "cassandra" ]; then
	...
	docker run -i --link="${NAME_PREFIX}redis:db" -p 9410:9410 -p 9903:9903 --name="${NAME_PREFIX}kinesis-collector" -t "${IMG_PREFIX}kinesis-collector"
elif [ $name == "query" ]; then
	echo "** Starting zipkin-query"
	docker run -i --link="${NAME_PREFIX}redis:db" -p 9411:9411 --name="${NAME_PREFIX}query" -t "${IMG_PREFIX}query"
elif [ $name == "web" ]; then
